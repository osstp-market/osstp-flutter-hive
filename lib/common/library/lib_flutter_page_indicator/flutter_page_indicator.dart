...
import '../lib_transformer_page_view/transformer_page_view.dart';
...
class ScalePainter extends BasePainter {
  ...
  void draw(Canvas canvas, double space, double size, double radius) {
    ...
    double progress = page - index;
    _paint.color = Color.lerp(widget.activeColor, widget.color, progress) ?? Colors.orangeAccent;
    //last
    canvas.drawCircle(
        Offset(radius + (index * (size + space)), radius), lerp(radius, radius * widget.scale, progress), _paint);
    //first
    _paint.color = Color.lerp(widget.color, widget.activeColor, progress) ?? Colors.pink;
  ...
  BasePainter _createPainer() {
    switch (widget.layout) {
      case PageIndicatorLayout.NONE:
        return NonePainter(widget, widget.controller.page, index, _paint);
      case PageIndicatorLayout.SLIDE:
        return SlidePainter(widget, widget.controller.page, index, _paint);
      case PageIndicatorLayout.WARM:
        return WarmPainter(widget, widget.controller.page, index, _paint);
      case PageIndicatorLayout.COLOR:
        return ColorPainter(widget, widget.controller.page, index, _paint);
      case PageIndicatorLayout.SCALE:
        return ScalePainter(widget, widget.controller.page, index, _paint);
      case PageIndicatorLayout.DROP:
        return DropPainter(widget, widget.controller.page, index, _paint);
      default:
